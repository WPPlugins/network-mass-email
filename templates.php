...
function nmef_loadonetemplate() {
	...
	else if ($wpdb->get_var($wpdb->prepare("SELECT name FROM {$table_name} WHERE name=%s", $_POST['selecttemplate'])) == $_POST['selecttemplate']) {		
		//select template row from db			
		$templateresult = $wpdb->get_row($wpdb->prepare("SELECT * FROM $table_name WHERE name=%s", $_POST['selecttemplate'] ));
...
function nmef_savetemplate() {
	...
	if ( ($subject != "") && ($templatename != "") && ($message != "") ) {
		//save template to DB
		if ( $wpdb->get_var($wpdb->prepare("SELECT name FROM {$table_name} WHERE name=%s", $_POST['templatename'])) == $templatename) {
...
function nmef_addupdate_template($name, $subject, $message) {
	...
	$checker = $wpdb->get_var( $wpdb->prepare("SELECT name FROM $wpdb->$table_name WHERE name=%s", $name ));
	if ($checker == NULL) { 
